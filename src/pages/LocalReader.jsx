...
import { 
  ...
  Search, X, Volume2, Target, Eye, Pause
} from 'lucide-react';
...
export default function LocalReader() {
  ...
  // Sync page view with global TTS chapter changes
  useEffect(() => {
    const handleGlobalTtsChange = (e) => {
      if (activeBook && e.detail && e.detail.bookId === activeBook.id) {
        if (e.detail.chapterIdx !== activeChapterIdx) {
  ...
  const handleParagraphDoubleClick = (pIdx, text) => {
    const content = activeBook?.chapters[activeChapterIdx]?.content || '';
    ...
    const titleText = activeBook.chapters[activeChapterIdx]?.title || '';
    if (titleText) {
      rawSentencesBefore.push(titleText);
    }
        ...
        <div className={`transition-colors duration-300 relative ${
          ...
          <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-white/5 flex items-center justify-between gap-2 bg-inherit">
            ...
            <div className="flex items-center gap-2.5">
              {/* Play Audio AI Controls container */}
              <div className="flex items-center gap-1 bg-black/10 border border-white/5 p-1 rounded-xl">
                <button
                ...
                {activeAudioObj && (
                  <>
                    <button
                      onClick={() => {
                        const isSameBook = activeAudioObj.book?.id === activeBook?.id;
                        if (isSameBook) {
                          if (activeAudioObj.chapterIdx !== activeChapterIdx) {
                            setActiveChapterIdx(activeAudioObj.chapterIdx);
                          } else {
                            const activeEl = document.getElementById('active-tts-sentence');
                            if (activeEl) {
                              activeEl.scrollIntoView({ behavior: 'smooth', block: 'center' });
